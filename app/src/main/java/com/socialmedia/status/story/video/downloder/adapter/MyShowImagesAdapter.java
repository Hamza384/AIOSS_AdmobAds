...
public class MyShowImagesAdapter extends PagerAdapter {
    private Context mContext;
    private ArrayList<File> imageList;
    private LayoutInflater inflater;
    MyFullViewActivity myFullViewActivity;

    public MyShowImagesAdapter(Context mContext, ArrayList<File> imageList, MyFullViewActivity myFullViewActivity) {
        this.mContext = mContext;
        this.imageList = imageList;
        this.myFullViewActivity = myFullViewActivity;
    ...
    public Object instantiateItem(@NotNull ViewGroup view, int position) {
        ...
        Glide.with(mContext).load(imageList.get(position).getPath()).into(imageView);
        ...
        im_delete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (b){
                    myFullViewActivity.deleteFileAA(position);
                }
            }
        });
        ...
        im_share.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String extension = imageList.get(position).getName().substring(imageList.get(position).getName().lastIndexOf("."));
                if (extension.equals(".mp4")){
                    shareVideo(mContext,imageList.get(position).getPath());
                }else {
                    shareImage(mContext,imageList.get(position).getPath());
                }
            }
        });
